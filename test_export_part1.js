(function(){
  O=O.filter(function(o){return o.code!='BTA_APPUI'&&o.code!='HTA_APPUI'&&!(o.n=='ECL'&&o.t=='fourreau')&&!(o.n=='TEL'&&o.t=='fourreau')});
  ...
  if(C.pts&&C.pts.AEP){
    var aepPts=C.pts.AEP.split(/\s+/).filter(Boolean);
    ['COMPTEUR','ARMOIRE'].forEach(function(x){if(aepPts.indexOf(x)<0)aepPts.push(x)});
    C.pts.AEP=aepPts.join(' ');
  }
  function exists(code){return O.some(function(o){return o.code==code})}
  if(!exists('AEP_COMPTEUR'))add('AEP','AEP_COMPTEUR','Equipements','equipement','compteur');
...
function expJSON(){deliver('land2map_validation_v39.json',JSON.stringify({version:'v39',date:new Date().toISOString(),objets:O.map(patch)},null,2))}